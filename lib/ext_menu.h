...
._v3_redisplay_menu;
	...
	print "Ci sono informazioni a proposito di:^^";
	for(i = 1: i <= lines: i++) {
		menu_item = i;
		indirect(EntryR);
	...
	if(menu_nesting == 1) {
		print "q: Torna al gioco^";
	} else {
		print "q: Menù precedente^";
	}

	for (::) {
		print "^Seleziona da 1 a ", lines, " o INVIO per mostrare ancora le opzioni.^";
		print "> ";
...
Constant NKEY__TX	   = "S = argomento successivo";
Constant PKEY__TX       = "P = precedente";
Constant QKEY1__TX      = "  Q = torna al gioco";
Constant QKEY2__TX      = "Q = menù precedente";
Constant RKEY__TX       = "INVIO = leggi l'argomento";
