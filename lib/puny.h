...
#Endif;
	...
	if(p_obj has container) {
		...
		if(p_obj hasnt open && p_obj hasnt transparent) {
			print " (chius", (_o) p_obj, ")";
			if(_newline) new_line;
		} else {
			if(p_obj has openable && (p_obj has transparent || _contents == 0)) {
				print " (che ", (IsorAre) p_obj, " ";
				if(p_obj has open) { print "apert", (_o) p_obj; _started = 1; }
				else { print "chius", (_o) p_obj; _started = 2; }
			}
			if(_contents == 0) {
				print (char) ' ';
				switch(_started) {
				0: print "(che ", (IsorAre) p_obj;
				1: print "ma";
				2: print "e";
				}
				print "vuot", (_o) p_obj, ")";
		...
		if(_newline) {
			...
			if(PrintContents(" (su cui ", p_obj, ISARE_BIT)) print (char) ')';
	} else if(_newline)
		new_line;
];
...
#Endif;
	...
	if(_last_obj) {
		if(_printed_any_objects ~= 0 && c_style & NEWLINE_BIT == 0) print " e ";
#Ifdef OPTIONAL_LIST_TOGETHER;
		if(_last_obj_was_LT_special)
			_PrintContentsPrintLTGroup(_last_obj);
		else
...
!		short_name  "tu",
		short_name  [; 
			if(caps_mode) { print "Tu"; rtrue; }
			print "tu";
			rtrue;
		],
		description "Splendido come sempre.",
		before NULL,
		after NULL,
		life NULL,
		each_turn NULL,
...
Object thedark "Buio"
	with
		initial 0,
		description "Non si riesce a vedere nulla!",
 		short_name 0;
#Endif;
