...
!     has light;
! ==============================================================================
! Modifica specifica della versione italiana:
!   Affinchè possano esistere "scenografie economiche" ("cheap scenery") con
!   articoli femminili (la strada, le strade), sono state create le costanti:
!       * CS_FEM: Per scenografie femminili singolari ("la strada")
!       * CS_FEM_THEM: Per scenografie femminili plurali ("le strade")
!
!   Esempio:
!
!     Object GloomyStreet "Strada Triste"
! 	    with
!		  description "Le case di questa zona sembrano essere state abbandonate in fretta e furia, come se una misteriosa tragedia avesse seminato
...
!		  s_to Library,
!  		  cant_go "Il tuo lavoro è a sud, anche se sei tentato di tornare a casa e dimenticarti di tutto.",
!  	    has light;
...
Constant CS_DEFAULT_MSG "Non c'è bisogno di preoccuparsene.";
#Endif;
...
Object CheapScenery "oggetto"
	with
		...
		parse_name [ _ret _pronoun;
			...
			if(lo_obj == self && _pronoun ~= CS_IT) lo_obj = 0;
			if(la_obj == self && _pronoun ~= CS_FEM) la_obj = 0;
			if(li_obj == self && _pronoun ~= CS_THEM) li_obj = 0;
			if(le_obj == self && _pronoun ~= CS_FEM_THEM) le_obj = 0;
			switch(_pronoun) {
			CS_THEM:
