...
[ ParseTime _i _j _k _flg _loop _dig _hr _mn;
	...
	_i = NextWord();
	if (_i == 'mezzogiorno' or 'mezzanotte') {   ! then case (e) applies
		if (_i == 'mezzanotte')
			_hr = 0;
		else
			_hr = 12;
...
._tw_diglph;
			...
			if ((_j == 'o^clock' or -1) || (_j == 'am' or 'pm')) { ! then case (c) applies
				...
				if (_k ~= -1000) {                     ! then case (b) applies
					...
					if (_i == 'veinticinque')
						_mn = 25;
					if (_i == 'mezza' or 'trenta')
						_mn = 30;
					if (_j == 'minuto' or 'minuti') {
...
#Ifdef STATUSLINE_SCORE;
	Extend 'aspetta'
		...
		* number 'minuto'/'minuti'                  -> WaitMinutes
		* number 'ora'/'ore'                      -> WaitHours
		* number 'mossa'/'mosse'/'turno'/'turni'       -> WaitMoves
		* number                                     -> WaitMoves;
#Ifnot;
	Extend 'aspetta'
