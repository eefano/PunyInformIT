...
#Ifndef VN_1644;
Message fatalerror "*** La libreria PunyInformIT necessita di Inform v6.44 o successive ***";
#Endif; ! VN_
...
Array direction_name_array static --> "direzione" "nord" "sud" "est" "ovest" "nordest" "nordovest" "sudest" "sudovest" "su" "giu" "entra" "esci";
...
Array direction_name_array static --> "direzione" "nord" "sud" "est" "ovest" "su" "giu" "entra" "esci";
...
Object Directions
	with
		description "Non si vede nulla di nuovo da quella parte.",
		short_name [;
...
#Ifndef OPTIONAL_SHIP_DIRECTIONS;
				if(_w == 'pavimento' or 'terra' or 'suolo') {
...
#Ifndef OPTIONAL_SHIP_DIRECTIONS;
				...
				@je _w 'u//' 'su' ?_matched_u;
#Endif;
#Ifdef OPTIONAL_FULL_DIRECTIONS;
				@je _w 'se' 'sudest' ?_matched_se;
				@je _w 'so' 'sudovest' ?_matched_sw;
				@je _w 'ne' 'nordest' ?_matched_ne;
				@je _w 'no' 'nordovest' ?_matched_nw;
#Endif;
				@je _w 'e//' 'est' ?_matched_e;
				@je _w 'o//' 'ovest' ?_matched_w;
				@je _w 's//' 'sud' ?_matched_s;
				@je _w 'n//' 'nord' ?_matched_n;
			}
